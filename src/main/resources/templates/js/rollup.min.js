...
function sousuo(src) {
    ...
    var selectvalue = ($("#select_info_").text());
    $("#select").find("option[text='"+selectvalue+"']").attr("selected", true);
    var selectsid = $("#select").val();
    if (selectsid == "")
        window.location.href = '/search?key=' + encodeURIComponent(xz);
    else
        window.location.href = '/search?key=' + encodeURIComponent(xz) + '&SelSID=' + selectsid + '&SelValue=' + selectvalue;
};

function sousuotwo(src) {
    var me = $(src);
    var xz = me.val();
    var def = me.attr("defvalue") || "";
    if (xz.length > 0 && xz == def) {
...
$(function () {
    ...
    $('#sousuotow').click(function () {
        sousuotwo($('#seachkeywordstow'));
        value = $('#seachkeywordstow').attr("defvalue");
    });
    SelectMenu();
    if (typeof (SID) != "undefined") {
        if (SID) {
            var div = $("div.leib2:first");
            var divs = $("div.leib2s:first");
            var divnews = $("div.leib2_news:first");
            var divcp = $("div.cp-l:first")
            if (SID.length < 10) {//一级分类

            }
            div.find("h3[sid='" + SID + "'],li[sid='" + SID + "']").addClass("cur");
            divs.find("h3[sid='" + SID + "'],li[sid='" + SID + "']").addClass("cur");
            divnews.find("h3[sid='" + SID + "'],li[sid='" + SID + "']").addClass("cur");
            divcp.find("h4[sid='" + SID + "'],li[sid='" + SID + "']").addClass("cur");
        }
    }
    action();
    var key = "defvalue"
