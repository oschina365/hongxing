<%@ page language="C#" autoeventwireup="true" inherits="NSW.Web.Manager.product_attribute, qwt" enableviewstate="false" enableviewstatemac="false" %>
    ...
    <form runat="server">
        ...
        <table class="nsw_pro_list" id="nsw_list_table" cellpadding="0" cellspacing="0" width="100%">
            ...
            <tr class="child_1 <%#Eval("ID").ToString().Equals("1")?" dn":"" %>">
                ...
                <td>
                    <div class="set_radios" oid="<%#Eval("ID") %>" typename="productattributecolumn" fileColumn="Enable">
                        <span><input type="radio" <%#(bool)Eval("Enable")?"checked=\"checked\"":""%>/><label></label></span>
                    </div>  
                </td>
                <td><span class="editor_warp" data-src='{"action":"alertOrder","dataType":"number","editor":"input_editor","type":"productattributecolumn","oid":"<%#Eval("ID") %>"}'><%#Eval("OrderID") %></span></td>
                <td>
                    <span><%#((DateTime)Eval("InputTime")).ToString("yyyy-MM-dd<br />HH:mm:ss")%></span>
                </td>
                <td class="nsw_cnt_area action_tx">
                    <span class="pro_dele editor_warp" data-src='{"action":"delete","editor":"delete_editor","type":"productattributecolumn","oid":"<%#Eval("ID") %>"}'><i></i>删除</span>
                </td>
            </tr>
