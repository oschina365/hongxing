...
public class DbQuery implements Closeable, AutoCloseable {
    ...
    public <T> T read(Class<T> beanClass, String sql, Object... params) {
        try {
            return (T) _g_runner.query(conn, sql, _IsPrimitive(beanClass)
                    ? _g_scaleHandler : new BeanHandler(beanClass), trimSQLParams(sql, params));
        } catch (SQLException e) {
            throw new DBException(e);
        }
    }
    ...
    public <T> List<T> query(Class<T> beanClass, String sql, Object... params) {
        try {
            return (List<T>) _g_runner.query(conn, sql, _IsPrimitive(beanClass)
                    ? _g_columnListHandler : new BeanListHandler(beanClass), trimSQLParams(sql, params));
        } catch (SQLException e) {
            throw new DBException(e);
        }
    }
