<%@ page language="C#" autoeventwireup="true" inherits="NSW.Web.Manager.project_column, qwt" enableviewstate="false" enableviewstatemac="false" %>
...
<div class="nsw_add_proclsify f_cb" style="padding-left:2%;">
	...
	<span class="clear_bd fast_in f_fl">
		<select class="pass_faq" name="cid">
			<%=NSW.Admin.Tools.AdminTools.FillColumnData("<option value='{id}'>{title}</option>", NSW.Web.API.ColumnType.Project.ToString())%>
		</select>
		<i class="revise_sub"></i>
	</span>
	...
	<span class="e_btn1 f_ml2 f_csp f_fl f_mr5 ajaxSave" data-src='{"action":"BatchAddColumn"}' style=" height:28px;line-height:28px; width:88px;"><i class="save_icon" style="margin:4px 7px 0 18px"></i>保 存</span>
	<span class="role_addbtn f_ml2 f_csp f_fl popUp" data-src='{"popup":"BatchAddColumnPopUp","action":"BatchAddColumn","title":"批量添加方案分类","type":"projectcolumn"}'>添 加</span>
    <input type="hidden" name="typename" value="projectcolumn" /><input type="hidden" name="mode" value="single" /></form>
</div>
<!-- 快速录入 结束 -->
    ...
    <form runat="server">
        ...
        <table class="nsw_pro_list grouping" id="nsw_list_table" cellpadding="0" cellspacing="0" width="100%">
            <tr id="tabHeader">
                ...
                <th axis="[Title]" style=" width:200px;">方案分类</th>  
...
<%--                        <i class="tir4"></i>--%>
                ...
                <td>
                <div class="hd_input" oid="<%#Eval("ID") %>" typename="projectcolumn" fileColumn="isCommend">
                ...
                <td>
                <div class="hd_input" oid="<%#Eval("ID") %>" typename="projectcolumn" fileColumn="isTop">
                ...
                <td>
                <div class="hd_input" oid="<%#Eval("ID") %>" typename="projectcolumn" fileColumn="isBest">
                ...
                <td>
                <div class="hd_input" oid="<%#Eval("ID") %>" typename="projectcolumn" fileColumn="Enable">
                ...
                <td><%#Eval("PostSum")%></td>
                <td><span class="editor_warp" data-src='{"action":"alertOrder","dataType":"number","editor":"input_editor","type":"projectcolumn","oid":"<%#Eval("ID") %>"}'><%#Eval("OrderID") %></span></td>
                <td>
                ...
                <td class="nsw_cnt_area action_tx <%#Eval("ID").ToString()=="1"?" f_noinfo":"" %>">
                    <a href='<%#MyTool.GetColumnURL(Container,NSW.Web.API.ColumnType.Project) %>' title='<%#Eval("Title") %>' target='_blank'><span class="pro_view"><i></i>预览</span></a>
                    <a href="##" onclick="<%#Eval("ID").ToString()=="1"?" ":"edit(event,"+Eval("ID")+",'action=copy')" %>"><span class="pro_copy"><i></i>复制</span></a>
                    <span class="pro_dele editor_warp" data-src='{"action":"delete","editor":"delete_editor","type":"projectcolumn","oid":"<%#Eval("ID") %>"}'><i></i>删除</span>
                </td>
                <% if (NSW.Mobile.MobileConfig.MEnableMobile == "true"){ %>
        ...
        <div class="nsw_tools_bar f_cb">
	        ...
	        <div class="batch_act f_fl f_ml20">
		        ...
		        <ul class="batch_dropup column">
			        ...
			        <li class="popUp" data-src='{"popup":"BatchAddColumnPopUp","action":"BatchAddColumn","title":"批量添加分类","type":"projectcolumn"}'>批量添加分类</li>
                    <li class="popUp" data-src='{"popup":"ListImportedColumnAttribute","type":"project"}'>批量导入属性</li>
		        </ul>
	        </div>
