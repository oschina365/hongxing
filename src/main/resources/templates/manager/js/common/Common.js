...
var loadJs = function (src, fn, doc) {
    ...
    var obj = win["object_"] || {};
    obj[file] = (obj[file] || 0) + 1;
    win["object_"] = obj;
    var Executive = function (file, fn, isTrue) {
        if (fn) {
            if (isTrue) {
                fn();
            } else {
    ...
    var _setTimeout = function (file, fn, max, index) {
        var key = file + "_interval" + index;
        if (win.loadjs_obj[key]) clearInterval(win.loadjs_obj[key]);
        win.loadjs_obj[key] = setInterval(function () {
            if (win.loadjs_obj[file] || max < 0) {
                if (win.loadjs_obj[key]) clearInterval(win.loadjs_obj[key]);
                Executive(file, fn, (win.loadjs_obj[file] = true));
            }
            max = max - 1;
        }, 100);
    }
    ...
    if (jss[src]) {
        if (fn) {
            if (jss[src] == "complete" || win.loadjs_obj[file]) {
                Executive(file, fn, win.loadjs_obj[file]);
            } else {
                _setTimeout(file, fn, 500, win["object_"][file]);
            }
        }
        return true;
    }
