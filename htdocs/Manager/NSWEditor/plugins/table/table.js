	...
	self.plugin.table = {
		...
		prop : function(isInsert) {
			var html = [
				...
				lang.backgroundImage + ' <input type="text" class="ke-input-text ke-input-url" style="width:150px;" name="bgimg" value="http://" />',
				'</div>',
				'</div>'
			].join('');
				...
				yesBtn : {
					...
					click : function(e) {
						...
						if (table) {
							...
							if(bgimg!==''&&bgimg!='http://'){
								table.css('background-image','url('+bgimg+')');
							}else{
			...
			if (table) {
				...
				bgimgBox.val((""+table[0].style.backgroundImage).replace("url(","").replace(")",""));
