<%@ page language="C#" autoeventwireup="true" inherits="NSW.Web.Manager.product_columns, qwt" enableviewstate="false" enableviewstatemac="false" %>
...
<div class="nsw_add_proclsify f_cb" style="padding-left:2%;">
	...
	<span class="clear_bd fast_in f_fl">
		<select class="pass_faq" name="cid">
			<%=NSW.Admin.Tools.AdminTools.FillColumnData("<option value='{id}'>{title}</option>", NSW.Web.API.ColumnType.Product.ToString())%>
		</select>
		<i class="revise_sub"></i>
	</span>
    ...
    <form runat="server">
        ...
        <table class="nsw_pro_list grouping" id="nsw_list_table" cellpadding="0" cellspacing="0" width="100%">
            <tr id="tabHeader">
                ...
                <th axis="[Title]" style=" width:200px;">产品分类</th>  
           ...
           <tr class="child_1 <%#Eval("ID").ToString().Equals("1")?" dn":"" %>" oid="<%#Eval("ID") %>" parentid="<%#Eval("ParentID") %>">
                ...
                <td>
                    <div class="hd_input" oid="<%#Eval("ID") %>" typename="productcolumn" fileColumn="isCommend">
                ...
                <td>
                    <div class="hd_input" oid="<%#Eval("ID") %>" typename="productcolumn" fileColumn="isTop">
                ...
                <td>
                <div class="hd_input" oid="<%#Eval("ID") %>" typename="productcolumn" fileColumn="isBest">
                ...
                <td>
                <div class="hd_input" oid="<%#Eval("ID") %>" typename="productcolumn" fileColumn="Enable">
                ...
                <td><%#Eval("PostSum")%></td>
                <td><span class="editor_warp" data-src='{"action":"alertOrder","dataType":"number","editor":"input_editor","type":"productcolumn","oid":"<%#Eval("ID") %>"}'><%#Eval("OrderID") %></span></td>
                <td>
                ...
                <td class="nsw_cnt_area action_tx <%#Eval("ID").ToString()=="1"?" f_noinfo":"" %>">
                    <a href='<%#MyTool.GetColumnURL(Container,NSW.Web.API.ColumnType.Product) %>' title='<%#Eval("Title") %>' target='_blank'><span class="pro_view"><i></i>预览</span></a>
                    <a href="##" onclick="<%#Eval("ID").ToString()=="1"?" ":"edit(event,"+Eval("ID")+",'action=copy')" %>"><span class="pro_copy"><i></i>复制</span></a>
                    <span class="pro_dele editor_warp" data-src='{"action":"delete","editor":"delete_editor","type":"productcolumn","oid":"<%#Eval("ID") %>"}'><i></i>删除</span>
                </td>
                <% if (NSW.Mobile.MobileConfig.MEnableMobile == "true"){ %>
