...
function LoginCheck(a, c) {
    ...
    $.post("/ajax.ashx?action=logincheck&t=" + Math.random(), {username: a, password: c}, function (d) {
        if (gav(d, "state") == "1") {
            $a(gav(d, "msg"));
...
function getTaoCanPrice(a) {
    $.post("/ajax.ashx?action=getTaoCanPrice&t=" + Math.random(), {IDList: a}, function (d) {
    ...
    $(document).ready(function () {
        $(".cbox").find("input").click(function () {
            ...
            if (!this.checked) {
                ...
                b = $("#txtIDList").val() + "," + this.value
            }
            $("#txtIDList").val(b);
            $("#TCount").html($("#txtIDList").val().split(",").length);
            getTaoCanPrice(b)
        })
    })
}
...
function initCommonHeader() {
    $.get("/ajax.ashx?action=initcommonheader&t=" + Math.random(), function (a) {
...
function getObjectOffset(d) {
    var a = $(d);
    var c = a.offset();
    var b = {top: c.top, left: c.left, bottom: c.top + a.height(), right: c.left + a.width()};
    return b
}

function productSelectCurrentPosition(a) {
    $(".nr h4[sid='" + a + "'],.nr h5[sid='" + a + "'],.nr li[sid='" + a + "']").addClass("cur")
}
...
function addToCart(f, c, a, e, b, h, d) {
    ...
    $.post("/ajax.ashx?action=addtocart&t=" + Math.random(), {pid: c, quti: g, atts: e, pidlist: b}, function (m) {
        var j = gav(m, "msg");
        var l = gav(m, "count");
        var k = gav(m, "state");
        if (d != null) {
        ...
        if (h && (gav(m, "state") == 1) && confirm("添加到购物车成功，是否马上刷新页面购物车页面？\r\n\r\n是 - 刷新本页面查看最新结果\r\n否 - 保留当前页面状态")) {
...
function changeQuantity(d, b, c) {
    ...
    showBgProc();
    $.post("/ajax.ashx?action=addtocart&t=" + Math.random(), {pid: b, quti: a, atts: c}, function (e) {
        if (gav(e, "state") == "1") {
...
function delCartProduct(c, a, b) {
    showBgProc();
    var d = 0;
    $.post("/ajax.ashx?action=addtocart&t=" + Math.random(), {pid: a, atts: b}, function (e) {
        if (gav(e, "state") == "1") {
...
function changeQuantity(c, b) {
    ...
    showBgProc();
    $.post("/ajax.ashx?action=addtocart&t=" + Math.random(), {pid: b, quti: a}, function (d) {
        if (gav(d, "state") == "1") {
            if (confirm("数量修改成功，是否马上刷新页面查看购物车结果？\n\n是 - 刷新页面查看结果\n否 - 保留当前页面状态")) {
                location.href = "cart.aspx?t=" + Math.random()
            } else {
...
function cancelOrder(b, a) {
    showBgProc();
    $.post("/ajax.ashx?action=cancelorder&t=" + Math.random(), {no: a}, function (c) {
        if (gav(c, "state") == "1") {
...
function delFavColumn(b, a) {
    showBgProc();
    $.post("/ajax.ashx?action=delfavfolumn&t=" + Math.random(), {oid: a}, function (c) {
        if (gav(c, "state") == "1") {
...
function delMyWish(c, a) {
    ...
    showBgProc();
    $.post("/ajax.ashx?action=delMyWishs&t=" + Math.random(), {ids: b}, function (e) {
        if (gav(e, "state") == "1") {
...
function delMyDownloads(c, a) {
    ...
    showBgProc();
    $.post("/ajax.ashx?action=delMyDownloads&t=" + Math.random(), {ids: b}, function (e) {
        if (gav(e, "state") == "1") {
            var d = $j(a).find("input[name=item]:checked");
            d.each(function (f) {
                $(this).parent().parent().remove()
...
function addFav(d, c, b, a) {
    ...
    $.post("/ajax.ashx?action=fav&t=" + Math.random(), {url: b, ptitle: c, column_id: a}, function (g) {
        var f = gav(g, "state");
        var e = gav(g, "msg");
        if (f == "1") {
...
function postComment(e, c, d) {
    ...
    $.post("/ajax.ashx?action=postcomment&t=" + Math.random(), {content: a, oid: c, verCode: b, mark: d}, function (h) {
        var k = gav(h, "state");
        var j = gav(h, "msg");
        if (k == "") {
            $a(h, -1)
...
function getHistory(a) {
    $.post("/ajax.ashx?action=gethistory&t=" + Math.random(), {mark: a}, function (b) {
        if (b.length == 0) {
            b = "<li>&nbsp;&nbsp;无浏览历史</li>"
        }
...
function getHelpStatic(a) {
    $.post("/ajax.ashx?action=helpsatisfaction&t=" + Math.random(), {oid: a}, function (k) {
...
function submitHelpUse(c, b) {
    ...
    $.post("/ajax.ashx?action=helpuseful&t=" + Math.random(), {oid: b, notion: a}, function (d) {
        if (gav(d, "state") == "0") {
            $a(gav(d, "msg"))
        } else {
            $a(gav(d, "msg"), 1);
...
function getSimilarArticle(a) {
    $.post("/ajax.ashx?action=getsmilararticle&t=" + Math.random(), {sid: a}, function (b) {
...
function userFeedback(c) {
    ...
    $.post("/ajax.ashx?action=userfeedback&t=" + Math.random(), {title: a, shortDesc: b}, function (f) {
        var e = gav(f, "state");
        var d = gav(f, "msg");
        if (e == "1") {
...
function changeFavColumn(c, a) {
    ...
    showProc(c);
    $.post("/ajax.ashx?action=changefavcolumn&t=" + Math.random(), {ids: b, targetId: c.value}, function (f) {
        var e = gav(f, "state");
        var d = gav(f, "msg");
        if (e == "1") {
            location.reload()
...
function sendInvitation(c) {
    ...
    showProc(c);
    $.post("/ajax.ashx?action=SendInvitation&t=" + Math.random(), {_email: a}, function (f) {
        var e = gav(f, "state");
        var d = gav(f, "msg");
        if (e == "1") {
...
function fillReportCategories() {
    $.get("/ajax.ashx?action=GetReportCategories&t=" + Math.random(), function (d) {
        var a = d.split("$$");
...
function fillLeavewordCategories() {
    $.get("/ajax.ashx?action=GetLeavewordCategories&t=" + Math.random(), function (d) {
        var a = d.split("$$");
        var c = '<option value="">请选择...</option>';
        for (var b = 0; b < a.length; ++b) {
            c += '<option value="' + a[b] + '">' + a[b] + "</option>"
...
function getAgentHelpStatic(a) {
    $.post("/ajax.ashx?action=agenthelpsatisfaction&t=" + Math.random(), {oid: a}, function (k) {
        var b = [parseInt(gav(k, "1")), parseInt(gav(k, "2")), parseInt(gav(k, "3"))];
        var g = b[0] + b[1] + b[2];
        if (g == 0) {
            g = 1
...
function submitAgentHelpUse(c, b) {
    showProc(c);
    var a = $("input[name=use]:checked").val();
    $.post("/ajax.ashx?action=agenthelpuseful&t=" + Math.random(), {oid: b, notion: a}, function (d) {
        if (gav(d, "state") == "0") {
            $a(gav(d, "msg"))
        } else {
            $a(gav(d, "msg"), 1);
...
function showMyWish(b) {
    ...
    $.post("/ajax.ashx?action=showMyWish&t=" + Math.random(), {oid: b}, function (d) {
...
function submitProductWishs(b, c, a) {
    ...
    var a = $j("txtNum").val();
    $.post("/ajax.ashx?action=postProductWishs&t=" + Math.random(), {oid: b, attr: c, num: a}, function (d) {
        if (gav(d, "state") == "1") {
            $a(gav(d, "msg"), 1)
        } else {
            $a(gav(d, "msg"))
...
function sendGetProductsNotify(c) {
    ...
    $.post("/ajax.ashx?action=sendGetProductsByColumn&t=" + Math.random(), {columnID: b, searchText: a}, function (d) {
...
function delAgentOrder(c, a) {
    ...
    showBgProc();
    $.post("/ajax.ashx?action=delAgentOrder&t=" + Math.random(), {ids: b}, function (e) {
        if (gav(e, "state") == "1") {
            var d = $j(a).find("input[name=item]:checked");
            d.each(function (f) {
                $(this).parent().parent().remove()
...
function AddCompare(b) {
    ...
    var a = $(b).val();
    $.post("/ajax.ashx?action=addCompare&t=" + Math.random(), {ids: a, flag: c}, function (g) {
        if (gav(g, "state") == "1") {
...
function DelOneCompare(b) {
    var a = $(b).attr("id");
    $.post("/ajax.ashx?action=delOneCompare&t=" + Math.random(), {ids: a}, function (c) {
        if (gav(c, "state") == "1") {
...
function addDownload(b, a) {
    $.post("/ajax.ashx?action=addDownload&t=" + Math.random(), {oid: b}, function (c) {
        if (gav(c, "state") == "1") {
            window.location = a
        } else {
            $a(gav(c, "msg"))
...
function getSubSiteInfos() {
    $.post("/ajax.ashx?action=subsiteinfos&t=" + Math.random(), {domain: document.domain}, function (c) {
        var a = gav(c, "phone");
        var b = gav(c, "address");
        $("#phones").html(a)
    })
...
function getSubSiteInfo() {
    $.post("/ajax.ashx?action=subsiteinfo&t=" + Math.random(), {domain: document.domain}, function (a) {
        if (a) {
            $("#site ul").html(a)
        } else {
            $("#site").remove()
