...
var zhezhao = '<div id="oran_full_bg" style="position: absolute; top: 0px; left: 0px; z-index: 9; background:none repeat scroll 0 0 #777777; opacity: 0.75;filter:progid:DXImageTransform.Microsoft.Alpha(opacity=75);"></div>';
function showZheZhao(isShow){
    var chen = top ? $(top.document.getElementById("oran_full_bg")) : $("#oran_full_bg");
	if(isShow){
		if(!chen.length){
            $(top.document.body||document.body).append(zhezhao);
            chen = top ? $(top.document.getElementById("oran_full_bg")) : $("#oran_full_bg");
		}
...
function showBianJi(id, title, html, fn) {
	...
	fn = fn||function(){};
	html = html||"";
	var lightbox = top?$(top.document.getElementById(id)):$("#"+id);
	if(!lightbox.length){
		$(top.document.body||document.body).append(bianji.replace("{id}",id));
		lightbox = top?$(top.document.getElementById(id)):$("#"+id);
	}
