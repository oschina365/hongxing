...
//输出
Cls_jsPage.prototype.GetUrlByPage = function (sU, sTPage, index) {
    if (index == 1) {
        return sU.replace("_" + sTPage, "");
    }
    return sU.replace(sTPage, index);
}
Cls_jsPage.prototype.Write = function () {
    ...
    if (iPI <= 1 && iPC <= 1) {
        ...
        FirstPU = PF_D;
        PrevPU = PP_D;
        NextPU = "<A class='o' href=\"" + this.GetUrlByPage(sU, sTPage, NextP) + "\">" + PN_E + "</A>";
        LastPU = "<A class='o' href=\"" + this.GetUrlByPage(sU, sTPage, iPC) + "\">" + PL_E + "</A>";
    }
    else if (iPI == iPC) {
        FirstPU = "<A class='o' href=\"" + this.GetUrlByPage(sU, sTPage, 1) + "\">" + PF_E + "</A>";
        PrevPU = "<A class='o' href=\"" + this.GetUrlByPage(sU, sTPage, PrevP) + "\">" + PP_E + "</A>";
        NextPU = PN_D;
        LastPU = PL_D;
    }
    else {
    ...
    if (iPC >= 1) {
        ...
        for (i = PageStart; i <= PageEnd; i++) {
            if (i != iPI) {
                XX = "<A class=\"" + CPT + "\" href=\"" + this.GetUrlByPage(sU, sTPage, i) + "\">" + PT_E.replace(sTPageNum, i) + "</A>";
